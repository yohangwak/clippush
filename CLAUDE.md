...
| Path | What |
|---|---|
| `bin/clippush` | The whole tool — one Bash script. Its `# ----------` headers mark the stages: arg parsing, `--setup`, the SSH connection, subcommands, reading the clipboard, upload, handing paths back |
...
- **Private by default.** Pushes use `umask 077` and `chmod go=`. Only `--share`
  loosens permissions, and it stays opt-in per push.
