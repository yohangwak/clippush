...
**Copy any file on your Mac → paste it into Claude Code running on a remote server.**

Screenshots, spreadsheets, PDFs, CSVs, logs — multiple files at once. One command bridges your local clipboard and your remote SSH session.
...
│                            │        │                                │
│  📋 remote paths now on    │        │  Claude Code:                  │
│     your clipboard         │ ─────▶ │    /clip  ← reads them all     │
└────────────────────────────┘        └────────────────────────────────┘
```
...
Existing tools ([clipssh](https://github.com/samuellawrentz/clipssh), [cc-clip](https://github.com/ShunmeiCho/cc-clip)) solve this for **screenshots only**. clippush handles **any file, and multiple files at once** — because half the time what you need to show Claude isn't a screenshot, it's a spreadsheet someone emailed you.
...
3. **Paste** — in remote Claude Code, either `Cmd+V` then Enter (your clipboard holds a ready-made "read these files" instruction, so Claude reads them right away) or just type `/clip`
...
📋 copied as a ready-to-paste instruction — Cmd+V into Claude Code, or type /clip
```
...
Yes — the core (push + path on clipboard) works with anything. Only the `/clip` command is Claude Code-specific.
